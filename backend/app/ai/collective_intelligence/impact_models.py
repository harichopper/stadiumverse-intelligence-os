...
class ROIAnalysis:
    ...
    def __post_init__(self):
        self.total_cost = (
            self.resource_cost + self.time_cost + self.personnel_cost + self.system_cost
        )

        self.total_benefit = (
            self.revenue_benefit
            + self.efficiency_benefit
            + self.satisfaction_benefit
            + self.risk_mitigation_benefit
            + self.carbon_benefit
        )
        ...
        if self.total_benefit > self.total_cost:
            ...
            self.payback_period_minutes = (
                self.total_cost / benefit_per_minute if benefit_per_minute > 0 else 120
            )
        else:
...
class ImpactAssessment:
    ...
    def calculate_impact_magnitude(self) -> float:
        ...
        improvement_factor = (
            abs(self.predicted_improvement - self.current_baseline) / 100.0
        )
        ...
        duration_factor = min(
            1.0, self.impact_duration_minutes / 120.0
        )  # 2 hours = max
...
class InterventionProposal:
    ...
    def calculate_intervention_efficiency(self) -> float:
        ...
        # Efficiency factors
        time_efficiency = 1.0 / max(
            1, self.estimated_implementation_time / 10.0
        )  # 10 min baseline
        complexity_efficiency = 1.0 - self.complexity_score

        if self.roi_analysis and self.roi_analysis.total_cost > 0:
            cost_efficiency = (
                self.roi_analysis.total_benefit / self.roi_analysis.total_cost
            )
        else:
            cost_efficiency = 1.0
    ...
    def get_priority_score(self) -> float:
        ...
        priority = (
            impact * 0.35 + efficiency * 0.25 + urgency * 0.25 + success_prob * 0.15
        )
    ...
    def get_expected_implementation_timeline(self) -> Dict[str, str]:
        ...
        timeline = {
            ...
            "preparation_complete": (start_time + timedelta(minutes=2)).strftime(
                "%H:%M"
            ),
            "implementation_start": (start_time + timedelta(minutes=3)).strftime(
                "%H:%M"
            ),
            "full_effect": (
                start_time + timedelta(minutes=self.estimated_implementation_time + 5)
            ).strftime("%H:%M"),
            "assessment_available": (
                start_time + timedelta(minutes=self.estimated_implementation_time + 15)
            ).strftime("%H:%M"),
        }
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "impact_analysis": {
                "assessments": [
                    assessment.to_dict() for assessment in self.impact_assessments
                ],
