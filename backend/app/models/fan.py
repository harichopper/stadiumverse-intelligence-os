...
class DigitalTwin(Base):
    ...
    memories = relationship("TwinMemory", back_populates="twin", cascade="all, delete-orphan")
    ...
    def get_personality_description(self) -> str:
        ...
        descriptions = [trait_descriptions.get(trait, trait.value) for trait in self.personality_traits[:3]]
        return f"Fan who {', '.join(descriptions)}"
    
    def update_learning_patterns(self, prediction_accuracy: float, behavior_change: Dict[str, Any]):
        ...
        accuracies = [entry["accuracy"] for entry in self.learning_history["accuracy_history"][-20:]]
    ...
    def to_memory_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "navigation": {
                ...
                "avg_speed": sum(self.navigation_speed_history[-10:]) / len(self.navigation_speed_history[-10:]) if self.navigation_speed_history else 1.2
            },
            "interactions": {
            ...
            "recent_memories": {
                ...
                for category in ["visits", "purchases", "interactions", "navigation", "emotions"]
...
class TwinMemory(Base):
    ...
    category = Column(String(50), nullable=False)  # visits, purchases, interactions, navigation, emotions
    ...
    related_entities = Column(ARRAY(String), default=[])  # volunteer IDs, facility IDs, etc.
...
class DigitalFan(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    fan_id = Column(String(20), unique=True, nullable=False, index=True)  # F001, F002, etc.
    ...
    accessibility_needs = Column(ENUM(AccessibilityNeed), default=AccessibilityNeed.NONE)
    ...
    stress_triggers = Column(ARRAY(String), default=[])  # What causes stress for this fan
    ...
    # Relationships
    twin = relationship("DigitalTwin", back_populates="fan", uselist=False, cascade="all, delete-orphan")
    ...
    def get_personality_summary(self) -> str:
        ...
        if self.persona_backstory:
            return self.persona_backstory[:200] + "..." if len(self.persona_backstory) > 200 else self.persona_backstory
        
        traits = [trait.value for trait in self.personality_traits] if self.personality_traits else []
        if not traits:
            return "Typical stadium visitor with adaptable preferences"
        
        return f"A {traits[0]} fan who is {traits[1] if len(traits) > 1 else 'adaptable'}"
    
    def update_emotion(self, new_emotion: FanEmotion, trigger: str = None, intensity: float = 1.0):
        ...
        if new_emotion in emotion_effects:
            ...
            for attribute, change in effects.items():
                if attribute in ["stress", "excitement", "fatigue"]:
                    ...
                    self.lost_probability = max(0.0, min(1.0, self.lost_probability + change))
    
    def add_memory_to_twin(self, category: str, title: str, data: Dict[str, Any], importance: float = 0.5):
    ...
    def generate_future_branches(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        branches = {
            ...
            "most_likely": {
                ...
                "key_events": ["Normal queue times", "Standard service", "Typical exit"]
            },
            "worst_case": {
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "age": self.age,
            "accessibility_needs": self.accessibility_needs.value if self.accessibility_needs else None,
            "favorite_team": self.favorite_team,
            
            # V2 Personality
            "personality_traits": [trait.value for trait in self.personality_traits] if self.personality_traits else [],
            ...
            "transportation": self.transportation.value if self.transportation else None,
            ...
            "predicted_exit_time": self.predicted_exit_time.isoformat() if self.predicted_exit_time else None,
    ...
    def update_emotion_based_on_event(self, event_type: str):
        ...
        if new_emotion:
            ...
            if new_emotion in [FanEmotion.STRESSED, FanEmotion.ANGRY, FanEmotion.FEARFUL]:
    ...
    def calculate_movement_speed(self) -> float:
        ...
        accessibility_factor = 0.7 if self.accessibility_needs != AccessibilityNeed.NONE else 1.0
...
class FanMovement(Base):
    ...
    def to_dict(self):
        return {
            "id": str(self.id),
            "fan_id": str(self.fan_id),
            "coordinates": [self.location.x, self.location.y] if self.location else None,
...
class FanPrediction(Base):
    ...
    def to_dict(self):
        return {
            ...
            "predicted_coordinates": [self.predicted_location.x, self.predicted_location.y] if self.predicted_location else None,
    ...
    def validate_prediction(self, actual_data: dict) -> bool:
        ...
        if self.prediction_type == PredictionType.MOVEMENT:
            ...
            if predicted_coords and actual_coords:
                ...
                distance = ((predicted_coords[0] - actual_coords[0]) ** 2 + 
                           (predicted_coords[1] - actual_coords[1]) ** 2) ** 0.5
