...
class OllamaProvider(AIProvider):
    ...
    def _extract_confidence_and_reasoning(self, content: str) -> tuple[float, str, List[str]]:
        """
        ...
        for i, line in enumerate(lines):
            ...
            if any(keyword in line_lower for keyword in ['reason', 'because', 'analysis', 'explanation']):
                # Capture this line and next few lines as reasoning
                reasoning_lines = [line]
                for j in range(i + 1, min(i + 4, len(lines))):
                    if lines[j].strip() and not any(keyword in lines[j].lower() for keyword in ['alternative', 'option', 'confidence']):
            ...
            if any(keyword in line_lower for keyword in ['alternative', 'option', 'instead', 'could also']):
        ...
        **kwargs
        ...
        try:
            ...
            async with session.post(url, json=payload) as response:
                ...
                confidence, reasoning, alternatives = self._extract_confidence_and_reasoning(content)
            ...
            return AIResponse(
                ...
                alternatives=["Retry request", "Check system status", "Use manual override"],
                token_count=0,
                latency_ms=latency_ms,
                model_used=self.model,
