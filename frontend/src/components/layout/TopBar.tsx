    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
        <div style={{ position: 'relative', width: 28, height: 28 }}>
          ...
          <svg viewBox="0 0 28 28" width="28" height="28" fill="none" style={{ position: 'relative', zIndex: 1 }}>
      ...
      >
        ...
        <span style={{ fontSize: 9, fontFamily: 'monospace', background: 'rgba(255,255,255,.06)', padding: '1px 5px', borderRadius: 4 }}>⌘K</span>
      </motion.button>
      ...
      <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        {/* weather */}
        <span style={{ fontSize: 11, color: '#CBD5E1' }}>🌤 22°C</span>

        {/* clock */}
        <Clock />

        {/* voice */}
        <motion.button onClick={() => setVoice(v => !v)} whileHover={{ scale: 1.1 }} whileTap={{ scale: .93 }}
          style={{ width: 30, height: 30, borderRadius: 8, background: voice ? 'rgba(59,130,246,.2)' : 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.08)', cursor: 'pointer', fontSize: 13, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          🎤
        </motion.button>

        {/* settings */}
        <motion.button whileHover={{ scale: 1.1, rotate: 60 }} transition={{ type: 'spring', stiffness: 300 }}
          style={{ width: 30, height: 30, borderRadius: 8, background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.08)', cursor: 'pointer', fontSize: 13, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ⚙️
        </motion.button>

        {/* avatar */}
        <div style={{ width: 30, height: 30, borderRadius: '50%', background: 'linear-gradient(135deg,#3B82F6,#8B5CF6)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, color: '#fff', flexShrink: 0 }}>FC</div>
      </div>
    </div>
  );
};
