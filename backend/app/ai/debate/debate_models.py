...
class DebateSession:
    ...
    def conclude_debate(self, decision: DebateDecision):
        ...
        self.total_duration_seconds = (self.concluded_at - self.started_at).total_seconds()
    ...
    def get_consensus_score(self) -> float:
        ...
        agreeing_agents = [pos for pos in self.agent_positions if pos.recommendation.lower() == most_common_rec]
        avg_confidence = sum(pos.confidence for pos in agreeing_agents) / len(agreeing_agents)
    ...
    def get_cost_estimates(self) -> Dict[str, float]:
        ...
        costs = [pos.estimated_cost for pos in self.agent_positions if pos.estimated_cost > 0]
    ...
    def get_high_confidence_positions(self, min_confidence: float = 0.8) -> List[AgentPosition]:
    ...
    def get_high_risk_positions(self) -> List[AgentPosition]:
        ...
        return [pos for pos in self.agent_positions if pos.risk_assessment.lower() in ["high", "critical"]]
    ...
    def update_analytics(self, debate_session: DebateSession):
        ...
        if debate_session.final_decision:
            ...
            self.average_consensus_score = (
                (self.average_consensus_score * (self.successful_decisions - 1) + new_consensus) 
                / self.successful_decisions
            )
            
            self.average_decision_quality = (
                (self.average_decision_quality * (self.successful_decisions - 1) + new_quality) 
                / self.successful_decisions
            )
