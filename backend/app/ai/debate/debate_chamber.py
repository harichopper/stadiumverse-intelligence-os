...
""",
    ...
    async def initiate_debate(
        ...
        if participating_agents is None:
            ...
            participating_agents = [
                role for role in AgentRole if role != AgentRole.COORDINATOR
            ]
    ...
    async def _gather_agent_positions(
        self, debate_session: DebateSession, agents: List[AgentRole]
    ):
        """Gather positions from all participating agents"""
...
"""
        ...
        for agent_role in agents:
            if agent_role in self.agent_system_prompts:
                task = self._get_agent_position(
                    ai_provider, agent_role, context_message, debate_session.context
                )
        ...
        for i, position in enumerate(positions):
            if isinstance(position, AgentPosition):
                ...
                logger.error(
                    f"Failed to get position from agent {agents[i]}: {position}"
                )
    ...
    async def _get_agent_position(
        ...
        try:
            ...
            response = await ai_provider.generate_debate_response(
                ...
                debate_context={"message": context_message, **debate_context},
            )

            # Parse response into structured position
            return self._parse_agent_response(
                agent_role, response.content, response.confidence
            )
    ...
    def _parse_agent_response(
        self, agent_role: AgentRole, content: str, ai_confidence: float
    ) -> AgentPosition:
        ...
        for line in lines:
            ...
            if any(
                ...
                for keyword in ["recommendation", "suggest", "propose"]
            ):
                ...
                keyword in line_lower for keyword in ["reason", "because", "analysis"]
            ):
                ...
                keyword in line_lower
                for keyword in ["alternative", "option", "instead"]
            ):
                current_section = "alternatives"
                alternatives.append(line)
                continue
            elif any(
                keyword in line_lower for keyword in ["cost", "resource", "budget"]
            ):
...
"""
        ...
        try:
            ...
            decision = self._parse_coordinator_decision(
                response.content, debate_session
            )
            ...
            fallback_decision = DebateDecision(
                ...
                implementation_steps=[
                    ...
                    "Review agent positions manually",
                ],
    ...
    def _parse_coordinator_decision(
        self, content: str, debate_session: DebateSession
    ) -> DebateDecision:
    ...
    def get_recent_debates(self, limit: int = 10) -> List[DebateSession]:
        ...
        return sorted(self.debate_history, key=lambda d: d.started_at, reverse=True)[
            :limit
        ]
    ...
    async def quick_consensus_check(
        self, topic: str, context: Dict[str, Any], agents: List[AgentRole] = None
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_confidence": sum(r["confidence"] for r in quick_responses)
            / len(quick_responses)
            if quick_responses
            else 0,
            "requires_full_debate": len(quick_responses)
            < len(agents) * 0.7,  # If >30% agents failed
        }
