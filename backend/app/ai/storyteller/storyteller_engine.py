...
"""
    ...
    async def generate_crowd_narrative(
        ...
        try:
            ...
            recommendation = await self._generate_smart_recommendation(crowd_data, prediction_data)
            
            # Use template or generate custom narrative
            if "crowd_prediction" in self.story_templates and style in self.story_templates["crowd_prediction"]:
    ...
    async def generate_intervention_story(
        ...
        try:
            ...
            intervention_data = {
                ...
                "resource_summary": self._summarize_resources(proposal.resource_requirements),
                "roi_summary": self._summarize_roi(proposal.roi_analysis) if proposal.roi_analysis else "Cost-benefit analysis pending",
                "risk_level": self._describe_risk_level(proposal.urgency_score),
                "problem_statement": self._generate_problem_statement(proposal, current_state),
            ...
            if style == NarrativeStyle.TECHNICAL:
                intervention_data.update({
                    ...
                    "fallback_plan": proposal.alternative_options[0] if proposal.alternative_options else "Manual override available",
                    "confidence_range": f"{max(50, int((proposal.success_probability - 0.1) * 100))}-{min(95, int((proposal.success_probability + 0.1) * 100))}",
                    "roi_details": self._format_roi_details(proposal.roi_analysis) if proposal.roi_analysis else "ROI calculation in progress",
            ...
            if template_key in self.story_templates and style in self.story_templates[template_key]:
    ...
    async def generate_debate_story(
        ...
        try:
            ...
            if include_agent_details and debate_session.agent_positions:
                high_confidence_positions = [pos for pos in debate_session.agent_positions if pos.confidence > 0.8]
            ...
            debate_data = {
                ...
                "final_decision": debate_session.final_decision.decision if debate_session.final_decision else "No decision reached",
                "implementation_plan": "; ".join(debate_session.final_decision.implementation_steps[:3]) if debate_session.final_decision and debate_session.final_decision.implementation_steps else "Implementation pending",
                "resource_impact": f"${debate_session.final_decision.roi_analysis.get('total_cost', 0):.0f}" if debate_session.final_decision and debate_session.final_decision.roi_analysis else "Cost analysis pending",
                "agent_highlights": "\n".join(agent_highlights) if agent_highlights else "All agents participated in consensus building",
                "decision_quality": int(debate_session.final_decision.get_decision_quality_score() * 100) if debate_session.final_decision else 0,
                "final_recommendation": debate_session.final_decision.decision if debate_session.final_decision else "Further analysis required",
                "confidence_level": int(debate_session.final_decision.confidence * 100) if debate_session.final_decision else 0,
            ...
            if template_key in self.story_templates and style in self.story_templates[template_key]:
                ...
                narrative = await self._generate_custom_narrative(
                    template_key,
                    {"debate_data": debate_data, "style": style.value}
                )
    ...
    async def generate_future_scenario_story(
        ...
        try:
            ...
            scenario_narrative_data = {
                ...
                "current_path": scenario_data.get("current_trajectory", "Standard operational trajectory"),
                "projected_outcome": likely_case.get("description", "Expected operational outcome"),
                "best_case_description": best_case.get("description", "Optimal conditions achieved"),
                "likely_case_description": likely_case.get("description", "Standard conditions maintained"),
                "worst_case_description": worst_case.get("description", "Challenging conditions managed"),
                "decision_points": self._identify_decision_points(scenario_data),
                "monitoring_priorities": self._identify_monitoring_priorities(scenario_data),
                "contingency_conditions": self._identify_contingency_triggers(scenario_data),
                "current_conditions": scenario_data.get("current_conditions", "normal operations"),
                "time_reference": f"Over the {timeframe}",
                "expected_conditions": likely_case.get("summary", "standard service levels"),
                "public_recommendation": self._generate_public_recommendation(scenario_data)
            }
...
"""
    ...
    def _summarize_impacts(self, impact_assessments: List) -> str:
        ...
        for assessment in impact_assessments[:3]:  # Top 3 impacts
            magnitude = abs(assessment.predicted_improvement - assessment.current_baseline)
            if magnitude > 20:
                level = "significant"
            elif magnitude > 10:
                level = "moderate"
            ...
            impact_summaries.append(f"{level} {assessment.category.value.replace('_', ' ')}")
    ...
    def _generate_problem_statement(self, proposal, current_state: Dict[str, Any]) -> str:
        ...
        return intervention_mapping.get(
            proposal.intervention_type.value,
    ...
    def _format_current_metrics(self, current_state: Dict[str, Any]) -> str:
        ...
        metrics = []
        key_metrics = ["total_fans", "avg_stress_level", "avg_queue_time", "operational_efficiency"]
    ...
    def _extract_debate_insights(self, debate_session: DebateSession) -> str:
        ...
        if debate_session.agent_positions:
            ...
            highest_conf = max(debate_session.agent_positions, key=lambda p: p.confidence)
            insights.append(f"{highest_conf.agent_name.value.title()} agent showed highest confidence ({highest_conf.confidence:.0%})")
            
            # Check for risk concerns
            risk_concerns = [p for p in debate_session.agent_positions if p.risk_assessment in ["high", "critical"]]
            if risk_concerns:
                insights.append(f"{len(risk_concerns)} agents identified elevated risk factors")
