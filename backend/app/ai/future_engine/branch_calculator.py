...
class BranchCalculator:
    ...
    async def calculate_future_branches(
        ...
        """
        logger.info(
            f"Calculating future branches for {prediction_horizon_minutes} minute horizon"
        )
        ...
        try:
            ...
            branch_set.decision_impact_analysis = await self._calculate_decision_impact(
                branch_set
            )
            ...
            fallback_branch = self._create_fallback_branch(
                BranchType.MOST_LIKELY, current_state, str(e)
            )
    ...
    async def _calculate_single_branch(
        ...
        try:
            ...
            analysis_prompt = self._create_branch_analysis_prompt(
                branch_type, current_state, scenario_context, horizon_minutes
            )
...
"""
    ...
    async def _parse_ai_branch_response(
        ...
        timeline_points = self._extract_timeline_points(
            ai_response, current_state, horizon_minutes
        )
        ...
        branch.intervention_opportunities = self._extract_intervention_opportunities(
            ai_response
        )
        ...
        branch.overall_satisfaction_score = self._calculate_overall_satisfaction(
            timeline_points
        )
        branch.resource_efficiency_score = self._calculate_resource_efficiency(
            timeline_points
        )
    ...
    def _extract_timeline_points(
        self, ai_response: str, current_state: Dict[str, Any], horizon_minutes: int
    ) -> List[TimelinePoint]:
        ...
        for minutes in range(0, horizon_minutes + 1, 5):
            ...
            point = TimelinePoint(
                ...
                crowd_density=self._apply_bounds(
                    point_data.get("crowd_density", current_crowd) * trend_modifier,
                    0.0,
                    1.0,
                ),
                avg_queue_time=max(
                    0, point_data.get("queue_time", current_queue) * trend_modifier
                ),
    ...
    def _extract_timepoint_data(self, ai_response: str, minutes: int) -> Dict[str, Any]:
        ...
        for line in lines:
            ...
            if f"{minutes} min" in line or f"t+{minutes}" in line:
                ...
                if crowd_match:
                    timepoint_data["crowd_density"] = (
                        float(crowd_match.group(1)) / 100.0
                    )
    ...
    def _extract_outcomes(
        self, ai_response: str, current_state: Dict[str, Any]
        ...
        for category, mapping in outcome_mappings.items():
            ...
            change_pct = (
                (predicted_value - mapping["current_value"]) / mapping["current_value"]
            ) * 100
    ...
    def _extract_critical_decisions(self, ai_response: str) -> List[str]:
        ...
        for line in lines:
            ...
            if any(
                ...
                for keyword in ["decision", "critical", "key", "important"]
            ):
    ...
    def _extract_intervention_opportunities(self, ai_response: str) -> List[str]:
        ...
        for line in lines:
            ...
            if any(
                keyword in line.lower()
                for keyword in ["intervention", "opportunity", "improve", "optimize"]
            ):
                if len(line) > 15:
    ...
    def _extract_risk_strategies(self, ai_response: str) -> List[str]:
        ...
        for line in lines:
            ...
            if any(
                keyword in line.lower()
                for keyword in ["mitigate", "prevent", "reduce", "strategy"]
            ):
                if len(line) > 15:
    ...
    def _extract_contributing_factors(
        self, ai_response: str, keywords: List[str]
    ) -> List[str]:
    ...
    def _generate_impact_description(
        self, category: OutcomeCategory, change_pct: float
    ) -> str:
    ...
    def _create_fallback_branch(
        self, branch_type: BranchType, current_state: Dict[str, Any], error_msg: str
    ) -> FutureBranch:
    ...
    async def _calculate_decision_impact(
        self, branch_set: FutureBranchSet
    ) -> Dict[str, Any]:
        ...
        branches = [
            b
            for b in [
                branch_set.best_case,
                branch_set.most_likely,
                branch_set.worst_case,
            ]
            if b
        ]
        ...
        return {
            "most_impactful_interventions": [
                {"intervention": interv, "frequency": count}
                for interv, count in common_interventions
            ],
            "decision_sensitivity": branch_set.variance_analysis.get(
                "volatility_score", 0.5
            ),
            "intervention_timing": "immediate"
            if any(b.overall_risk_score > 0.7 for b in branches)
            else "planned",
        }
