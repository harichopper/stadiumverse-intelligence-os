...
class InterventionCalculator:
    ...
    async def calculate_intervention_metrics(
        ...
        baseline = self.resource_costs.get(
            proposal.intervention_type, {"cost": 10.0, "time": 5, "disruption": 0.2}
        )
...
"""
    ...
    def calculate_intervention_roi(
        ...
        revenue_benefit = self._calculate_revenue_benefit(
            proposal, metrics, current_state
        )
        efficiency_benefit = self._calculate_efficiency_benefit(
            proposal, metrics, current_state
        )
        satisfaction_benefit = self._calculate_satisfaction_benefit(
            proposal, metrics, current_state
        )
        risk_mitigation_benefit = self._calculate_risk_benefit(
            proposal, metrics, current_state
        )
    ...
    def _calculate_revenue_benefit(
        ...
        revenue_impacts = [
            ...
            in [ImpactCategory.REVENUE_INCREASE, ImpactCategory.EFFICIENCY_GAIN]
        ]
    ...
    def _calculate_efficiency_benefit(
        ...
        for impact in proposal.impact_assessments:
            if impact.category == ImpactCategory.WAIT_TIME_REDUCTION:
                improvement = abs(
                    impact.predicted_improvement - impact.current_baseline
                )
    ...
    def _calculate_satisfaction_benefit(
        ...
        for impact in satisfaction_impacts:
            improvement = abs(impact.predicted_improvement - impact.current_baseline)
            benefit = (
                impact.fans_affected * 5.0 * (improvement / 100.0) * impact.confidence
            )
            total_benefit += benefit
    ...
    def _calculate_risk_benefit(
        ...
        risk_impacts = [
            ...
            for impact in proposal.impact_assessments
            if impact.category
            in [ImpactCategory.RISK_MITIGATION, ImpactCategory.SAFETY_IMPROVEMENT]
        ]
    ...
    def rank_interventions_by_efficiency(
        self, proposals: List[InterventionProposal], current_state: Dict[str, Any]
        ...
        for proposal in proposals:
            try:
                ...
                roi_analysis = self.calculate_intervention_roi(
                    proposal, metrics, current_state
                )
                efficiency_score = self.calculate_intervention_efficiency_score(
                    proposal, roi_analysis, metrics
                )

                ranked_interventions.append(
                    (proposal, efficiency_score, metrics, roi_analysis)
                )

            except Exception as e:
                logger.error(
                    f"Error calculating efficiency for proposal {proposal.proposal_id}: {e}"
                )
                continue
...
"""
    ...
    def record_intervention_outcome(
        self, proposal_id: str, actual_outcome: Dict[str, Any], success: bool
    ):
        ...
        if intervention_type:
            ...
            for key, value in outcome.items():
                ...
                if isinstance(value, (int, float)):
                    self.success_patterns[intervention_type]["factors"][key].append(
                        value
                    )
    ...
    def _update_failure_patterns(self, outcome: Dict[str, Any]):
        ...
        if intervention_type:
            ...
            for key, value in outcome.items():
                ...
                if isinstance(value, (int, float)):
                    self.failure_patterns[intervention_type]["factors"][key].append(
                        value
                    )
    ...
    def get_learning_insights(self) -> Dict[str, Any]:
        ...
        for pattern_type, data in self.success_patterns.items():
            total_attempts = data["count"] + self.failure_patterns.get(
                pattern_type, {}
            ).get("count", 0)
        ...
        insights["most_successful_types"] = sorted(
            type_success_rates.items(), key=lambda x: x[1], reverse=True
        )[:5]
