...
from ...models.fan import DigitalFan, DigitalTwin
...
class TimelineSnapshot:
    ...
    timestamp: datetime
    minutes_offset: int  # Minutes from current time (negative = past, positive = future)
...
class TimelineEngine:
    ...
    def __init__(self):
        self.timeline_snapshots: Dict[int, TimelineSnapshot] = {}  # offset_minutes -> snapshot
    ...
    async def generate_timeline_snapshot(
        ...
        try:
            ...
            logger.info(f"Generated timeline snapshot for offset {minutes_offset} minutes")
            return snapshot
            
        except Exception as e:
            logger.error(f"Error generating timeline snapshot: {e}")
            return self._create_fallback_snapshot(target_time, minutes_offset, current_state)
    ...
    async def _generate_future_snapshot(
        ...
        predicted_fans = await self._predict_fan_states(
            current_state,
            minutes_offset
        )
        ...
        return TimelineSnapshot(
            ...
            metadata={
                ...
                "branch_used": likely_branch.branch_type.value if likely_branch else "fallback"
            }
        )
    ...
    def _generate_present_snapshot(
        ...
        predictions = {
            ...
            "immediate_recommendations": self._generate_immediate_recommendations(current_state),
        ...
        return TimelineSnapshot(
            ...
            metadata={
                "generation_method": "current_state",
                "data_source": "live"
            }
        )
    ...
    async def _predict_stadium_state(
        ...
        if branch and branch.timeline_points:
            ...
            predicted_state.update({
                "crowd_density": min(1.0, current_state.get("crowd_density", 0.5) + (time_factor * 0.2)),
                "avg_queue_time": max(0, current_state.get("avg_queue_time", 5) + (time_factor * 2)),
                "fan_satisfaction": max(0, current_state.get("fan_satisfaction", 75) - (time_factor * 5)),
                "medical_risk_level": min(1.0, current_state.get("medical_risk_level", 0.1) + (time_factor * 0.1)),
                "revenue_rate": current_state.get("revenue_rate", 100) + (time_factor * 20),
                "volunteer_utilization": min(1.0, current_state.get("volunteer_utilization", 0.6) + (time_factor * 0.1)),
                "transport_efficiency": max(0.3, current_state.get("transport_efficiency", 0.8) - (time_factor * 0.1))
            })
    ...
    async def _predict_fan_states(
        ...
        for i in range(min(20, fan_count)):  # Predict top 20 fans for performance
            ...
            predicted_fans[fan_id] = {
                ...
                "confidence": 0.7 - (minutes_offset * 0.01)  # Confidence decreases over time
            }
    ...
    async def _predict_volunteer_states(
        ...
        for i in range(volunteer_count):
            ...
            predicted_volunteers[volunteer_id] = {
                ...
                "predicted_assignment": self._predict_volunteer_assignment(i, minutes_offset),
                "predicted_location": self._predict_volunteer_location(i, minutes_offset),
                "predicted_workload": self._predict_volunteer_workload(i, minutes_offset),
...
"""
    ...
    def _cleanup_timeline_snapshots(self):
        ...
        if len(self.timeline_snapshots) > (self.max_past_snapshots + self.max_future_snapshots):
            ...
            for offset in sorted_offsets:
                if offset < -self.max_past_snapshots or offset > self.max_future_snapshots:
    ...
    def _predict_fan_activity(self, fan_index: int, minutes_offset: int) -> str:
        ...
        activities = ["watching", "queuing", "eating", "walking", "resting", "exiting"]
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cache_range": {
                "min_offset": min(self.timeline_snapshots.keys()) if self.timeline_snapshots else 0,
                "max_offset": max(self.timeline_snapshots.keys()) if self.timeline_snapshots else 0
            }
        }
