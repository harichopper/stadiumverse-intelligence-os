...
class Volunteer(Base):
    ...
    def to_dict(self):
        ...
        return {
            ...
            "zone_assignment": str(self.zone_assignment)
            if self.zone_assignment
            else None,
    ...
    def is_available_for_task(
        self, required_skills: list = None, languages: list = None
    ) -> bool:
        ...
        if required_skills:
            volunteer_skills = (
                [skill.value for skill in self.skills] if self.skills else []
            )
    ...
    def calculate_distance_to(self, target_location: tuple) -> float:
        ...
        distance = (
            (dx * meters_per_degree_lng) ** 2 + (dy * meters_per_degree_lat) ** 2
        ) ** 0.5
        return distance
...
class VolunteerTask(Base):
    ...
    )  # medical_assistance, crowd_control, information, etc.
